...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  picnic-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  picnic-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "delivery_slots", "lists", "payment"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  picnic-pp-cli workflow status

  # Show status as JSON
  picnic-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("picnic-pp-cli")
			}
