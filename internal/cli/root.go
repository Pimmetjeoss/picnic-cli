...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "picnic-pp-cli",
		Short: `Picnic groceries CLI with offline SQLite store, FTS search, slot-watch, reorder, and spend-trend.`,
		Long: `Picnic groceries CLI with offline SQLite store, FTS search, slot-watch, reorder, and spend-trend.
...
Health check: run 'picnic-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("picnic-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'picnic-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDeliveriesCmd(flags))
	// `/lists` and `/my_store` endpoints were removed from Picnic's API
	// (replaced by Fusion pages). The commands stay in the binary so
	// any agent that calls them gets a clear error rather than "unknown
	// command", but they're hidden from --help to reduce surface noise.
	listsCmd := newListsCmd(flags)
	listsCmd.Hidden = true
	...
	categoriesCmd := newCategoriesPromotedCmd(flags)
	categoriesCmd.Hidden = true
	rootCmd.AddCommand(categoriesCmd)
	rootCmd.AddCommand(newDeliverySlotsPromotedCmd(flags))
	rootCmd.AddCommand(newMgmPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-written novel commands (not generator-emitted).
	rootCmd.AddCommand(newPicnicSyncCmd(flags))
	rootCmd.AddCommand(newSQLCmd(flags))
	rootCmd.AddCommand(newSinceCmd(flags))
	rootCmd.AddCommand(newSlotWatchCmd(flags))
	rootCmd.AddCommand(newPantryRecurringCmd(flags))
	rootCmd.AddCommand(newSpendTrendCmd(flags))
