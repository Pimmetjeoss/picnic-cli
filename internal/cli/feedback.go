...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".picnic-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PICNIC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PICNIC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "picnic-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.picnic-pp-cli/feedback.jsonl.
When ` + "`PICNIC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PICNIC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  picnic-pp-cli feedback list
  picnic-pp-cli feedback list --limit 5
  picnic-pp-cli feedback list --json`,
