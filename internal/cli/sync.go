...
Exit codes & warnings:
  ...
  picnic-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  picnic-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  picnic-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  picnic-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  picnic-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"categories":     "/my_store",
		"delivery_slots": "/cart/delivery_slots",
		"lists":          "/lists",
		"payment":        "/user/wallet/transactions",
	}
