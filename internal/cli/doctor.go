...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  picnic-pp-cli doctor
  picnic-pp-cli doctor --json
  picnic-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export PICNIC_AUTH_KEY=<your-key>"
					report["auth_docs_url"] = "https://picnic.app"
				} else {
					authConfigured = true
			...
			if os.Getenv("PICNIC_AUTH_KEY") != "" {
				authEnvSet = append(authEnvSet, "PICNIC_AUTH_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PICNIC_AUTH_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("picnic-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'picnic-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'picnic-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'picnic-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'picnic-pp-cli sync' to refresh."
	}
	return report
}
