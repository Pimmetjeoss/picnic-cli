...
type Config struct {
	...
	Path          string            `toml:"-"`
	PicnicAuthKey string            `toml:"auth_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PICNIC_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "picnic-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("PICNIC_AUTH_KEY"); v != "" {
		cfg.PicnicAuthKey = v
		cfg.AuthSource = "env:PICNIC_AUTH_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.PicnicAuthKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("PICNIC_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
