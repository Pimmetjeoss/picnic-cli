...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "picnic-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			req.Header.Set("x-picnic-auth", authHeader)
		}
		// Picnic-specific: /pages/*, /deliveries/*, and /profile-menu require
		// the agent + device-id headers. The mobile app sends them on every
		// request; the storefront enforces them on these particular surfaces
		// (HTTP 500 / 403 otherwise). Setting them unconditionally is cheap
		// and matches what the maintained MRVDH and simonmartyr wrappers do.
		req.Header.Set("x-picnic-agent", "30100;1.15.243-18832;")
		req.Header.Set("x-picnic-did", "picnic-pp-cli-91A8")
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "x-picnic-auth", maskToken(authHeader))
	}
