...
This CLI was built from community wrappers and the live Picnic NL API. The current command surface covers the endpoints listed under "What this CLI can do" above, but Picnic also has surface areas the CLI doesn't yet wrap:

- **Recipes** (`/pages/meals-page-root`, `/pages/recipe-details-page-root?recipe_id=`) — no command yet.
- **Parcels** (`/parcels`), **customer service** (`/cs-contact-info`, `/messages`, `/reminders`) — no command yet.
- **Aanbiedingen promo labels** — `aanbiedingen` returns 162 products with prices, but the "1+1 gratis" / "10% korting" labels live in a PML container one level above each tile and aren't yet attached. Prices and names are reliable; promo-label association is a follow-up.
- **`lists` and `categories` (`my_store`)** — endpoints removed by Picnic; commands stay hidden and return a clean 404.
- **`articles category`** — endpoint also removed; same.

If you hit a Picnic feature that's missing, an issue or PR with the path you discovered is welcome.
